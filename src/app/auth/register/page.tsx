...
function RegisterForm() {
    ...
    const buildPayload = (action: string) => {
        const base = { action, type: registrationType, email, phoneNumber, password, name }
        if (action === "verify-otp") Object.assign(base, { otp })
        if (registrationType === "patient") {
            return { ...base, regionCode, consentAccepted }
        }
        return {
            ...base,
            licenseNumber, licenseExpiry, specialty,
        ...
        if (step === "form") {
            ...
            try {
                const res = await fetch("/api/auth/register", {
                    ...
                    body: JSON.stringify(buildPayload("send-otp")),
                })
                const data = await res.json()
                if (!res.ok) { setErrorMessage(data.error || "Failed to send OTP") }
                else { setSuccessMessage(data.message); setStep("otp") }
            ...
            try {
                const res = await fetch("/api/auth/register", {
                    method: "POST",
                    headers: { "Content-Type": "application/json" },
                    body: JSON.stringify(buildPayload("verify-otp")),
                })
                const data = await res.json()
                if (!res.ok) { setErrorMessage(data.error || "Registration failed") }
                else { setSuccessMessage(data.message); setTimeout(() => router.push("/auth/login"), 2000) }
            } catch { setErrorMessage("An unexpected error occurred.") }
            finally { setIsLoading(false) }
        }
    }
            ...
            <div className={`w-full max-w-lg relative z-10 transition-all duration-700 ${mounted ? "animate-fade-in-up" : "opacity-0"}`}>
                ...
                <div className="card-glass p-8 shadow-xl shadow-black/[0.03]">
                    <div className="text-center mb-6">
                        ...
                        <p className="text-sm text-slate-500">Join our clinician-verified screening platform</p>
                    </div>
                    ...
                    <div className="relative flex gap-1 p-1 bg-slate-100/80 backdrop-blur-sm rounded-xl mb-6">
                        <div
                            ...
                            style={{ left: registrationType === "patient" ? "4px" : "50%", width: "calc(50% - 4px)" }}
                        />
                        <button type="button" onClick={() => setRegistrationType("patient")}
                            className={`relative z-10 flex-1 py-2.5 px-4 rounded-lg text-sm font-medium transition-colors duration-200 flex items-center justify-center gap-2 ${registrationType === "patient" ? "text-slate-900" : "text-slate-500 hover:text-slate-700"}`}>
                            <User className="w-4 h-4" /> Patient
                        </button>
                        <button type="button" onClick={() => setRegistrationType("doctor")}
                            className={`relative z-10 flex-1 py-2.5 px-4 rounded-lg text-sm font-medium transition-colors duration-200 flex items-center justify-center gap-2 ${registrationType === "doctor" ? "text-slate-900" : "text-slate-500 hover:text-slate-700"}`}>
                    ...
                    {successMessage && (
                        <div className="flex items-center gap-3 p-4 rounded-xl bg-green-50/80 border border-green-200 mb-6 animate-scale-in backdrop-blur-sm">
                            ...
                            <div>
                                ...
                                {step !== "otp" && <p className="text-xs text-green-600 mt-0.5">Redirecting to login...</p>}
                            </div>
                        </div>
                    )}
                    {errorMessage && (
                    ...
                    {step === "form" ? (
                        <>
                            <form onSubmit={handleSubmit} className="space-y-4">
                                ...
                                <div>
                                    ...
                                    <div className="relative">
                                        ...
                                        <input id="name" type="text" value={name} onChange={(e) => setName(e.target.value)}
                                            ...
                                            className={inputClass("name")} placeholder={registrationType === "doctor" ? "Dr. Jane Smith" : "Jane Smith"} required />
                                    </div>
                                </div>
                                {/* Email */}
                                <div>
                                    ...
                                    <div className="relative">
                                        ...
                                        <input id="email" type="email" value={email} onChange={(e) => setEmail(e.target.value)}
                                            ...
                                            className={inputClass("email")} placeholder="you@example.com" required />
                                    </div>
                                </div>
                                {/* Phone */}
                                <div>
                                    ...
                                    <div className="relative">
                                        ...
                                        <input id="phone" type="tel" value={phoneNumber} onChange={(e) => setPhoneNumber(e.target.value)}
                                            ...
                                            className={inputClass("phone")} placeholder="+1234567890" required pattern="^\+?[1-9]\d{1,14}$" />
                                    </div>
                                </div>
                                {/* Region */}
                                <div>
                                    ...
                                    <div className="relative">
                                        ...
                                        <select id="region" value={regionCode} onChange={(e) => setRegionCode(e.target.value)}
                                            ...
                                            className={inputClass("region")} required>
                                ...
                                {/* Doctor fields */}
                                <div className={`space-y-4 overflow-hidden transition-all duration-500 ease-out ${registrationType === "doctor" ? "max-h-[500px] opacity-100" : "max-h-0 opacity-0"}`}>
                                    ...
                                    <div>
                                        ...
                                        <div className="relative">
                                            ...
                                            <input id="license" type="text" value={licenseNumber} onChange={(e) => setLicenseNumber(e.target.value)}
                                                onFocus={() => setFocusedField("license")} onBlur={() => setFocusedField(null)}
                                                className={inputClass("license")} placeholder="MED-XXXX-XXXX" required={registrationType === "doctor"} />
                                        </div>
                                    </div>
                                    <div>
                                        ...
                                        <div className="relative">
                                            ...
                                            <input id="expiry" type="date" value={licenseExpiry} onChange={(e) => setLicenseExpiry(e.target.value)}
                                                onFocus={() => setFocusedField("expiry")} onBlur={() => setFocusedField(null)}
                                                className={inputClass("expiry")} required={registrationType === "doctor"} />
                                        </div>
                                    </div>
                                    <div>
                                        ...
                                        <div className="relative">
                                            ...
                                            <select id="specialty" value={specialty} onChange={(e) => setSpecialty(e.target.value)}
                                                ...
                                                className={inputClass("specialty")} required={registrationType === "doctor"}>
                                ...
                                <div>
                                    ...
                                    <div className="relative">
                                        ...
                                        <input id="password" type={showPassword ? "text" : "password"} value={password}
                                            ...
                                            className={`${inputClass("password")} pr-10`} placeholder="••••••••" required minLength={8} />
                                ...
                                <div>
                                    ...
                                    <div className="relative">
                                        ...
                                        <input id="confirmPassword" type="password" value={confirmPassword}
                                            ...
                                            className={inputClass("confirmPassword")} placeholder="••••••••" required />
                                ...
                                {/* Patient consent */}
                                <div className={`overflow-hidden transition-all duration-500 ease-out ${registrationType === "patient" ? "max-h-[200px] opacity-100" : "max-h-0 opacity-0"}`}>
                                ...
                                {/* Doctor notice */}
                                <div className={`overflow-hidden transition-all duration-500 ease-out ${registrationType === "doctor" ? "max-h-[200px] opacity-100" : "max-h-0 opacity-0"}`}>
                                ...
                                <button type="submit" disabled={isLoading}
                                    ...
                                    {isLoading ? (
                                        <><span className="spinner" /><span>Sending OTP...</span></>
                                    ) : (
                                        <><Sparkles className="w-4 h-4" /><span>Continue with Registration</span><ArrowRight className="w-4 h-4" /></>
                                    )}
                                </button>
                            </form>
                        ...
                        <form onSubmit={handleSubmit} className="space-y-4">
                            <div className="p-4 bg-blue-50/80 backdrop-blur-sm rounded-xl border border-blue-200 mb-2">
                            ...
                            <div>
                                <label htmlFor="otp" className={labelClass("otp")}>One-Time Password (OTP)</label>
                                <div className="relative">
                                    <Lock className={iconClass("otp")} />
                                    <input id="otp" type="text" maxLength={6} value={otp}
                                        onChange={(e) => setOtp(e.target.value.replace(/\D/g, ''))}
                                        onFocus={() => setFocusedField("otp")} onBlur={() => setFocusedField(null)}
                                        className={`${inputClass("otp")} text-center tracking-widest font-mono text-lg`}
                            ...
                            <button type="submit" disabled={isLoading || otp.length !== 6}
                                ...
                                {isLoading ? (
                                    ...
                                    <><CheckCircle className="w-4 h-4" /><span>Verify &amp; Complete Registration</span></>
                                )}
                            </button>
                            <button type="button" onClick={() => { setStep("form"); setErrorMessage(""); setSuccessMessage("") }}
                    ...
                    <p className="text-center text-sm text-slate-600 mt-6">
                        ...
                        <Link href="/auth/login" className="font-medium text-blue-600 hover:text-blue-700 transition-colors inline-flex items-center gap-1 hover:gap-2">
